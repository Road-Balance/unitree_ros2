...
void gait_type_cb(const std_msgs::msg::UInt8::SharedPtr msg)
{
	gaitType = msg->data;
	std::cout << unsigned(gaitType) << std::endl;
}
...
int mainHelper(int argc, char *argv[], TLCM &roslcm)
{
	...
	auto gait_type_sub = node->create_subscription<std_msgs::msg::UInt8>("gait_type", 10, gait_type_cb);
	...
	while (rclcpp::ok())
	{
		...
		switch (mode)
		{
		case 0: // 0. idle, default stand
			SendHighROS.velocity[0] = 0;
			break;
		case 1: // 1. force stand (controlled by dBodyHeight + ypr)
			SendHighROS.body_height = 0.2;
			break;
		case 2: // 2. target velocity walking (controlled by velocity + yawSpeed)
			SendHighROS.gait_type = gaitType; // TODO : uint type check
			SendHighROS.velocity[0] = 0.4f; // -1  ~ +1
			SendHighROS.yaw_speed = 2;
			SendHighROS.foot_raise_height = 0.1;
			break;
		case 3: // 3. target position walking (controlled by position + ypr[0])
			break; 
		case 4: // 4. path mode walking (reserve for future release)
			break; 
		case 5: // 5. position stand down. 
			break; 
		default:
